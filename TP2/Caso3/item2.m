...
for ki=1:KMAX
    ...
    u1(ki)=-K*x+KI*ve(ki+1); color='--r'; %Sin observador de estados
    u1_o(ki)=-K*x_hat+KI*ve_o(ki+1); color_o='b'; %Con observador de estados

    % Integracion de Euler para sistema sin observador:
    for kii=1:Ts/h
        u(ii)=u1(ki);
...
legends_c = ["sin Observador","con Observador"];

subplot(3,2,1);
hold on;
plot(t,fi_p_o,color_o,'Linewidth',1.2);
...
subplot(3,2,2);
hold on;
plot(t,fi_o,color_o,'Linewidth',1.2);
plot(t,fi,color,'Linewidth',1.2);
grid on;
...
subplot(3,2,3);
hold on;
plot(t,p_o,color_o,'Linewidth',1.2);
plot(t,p,color,'Linewidth',1.2);
grid on;
...
subplot(3,2,4);
hold on;
plot(t,p_p_o,color_o,'Linewidth',1.2);
...
subplot(3,1,3);
hold on;
plot(t,u_o,color_o,'Linewidth',1.2);
